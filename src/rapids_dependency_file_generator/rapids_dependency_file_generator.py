...
def make_dependency_file(
    ...
    file_contents = textwrap.dedent(
        f"""\
        # This {'section' if file_type == str(OutputTypes.CONDA_META) else 'file'} is generated by `{cli_name}`.
    ...
    if file_type == str(OutputTypes.CONDA_META):
        file_contents += yaml.dump({"requirements": dependencies})
...
def get_filename(file_type, file_prefix, matrix_combo):
    ...
    if file_type == str(OutputTypes.CONDA_META):
        if matrix_combo:
            raise ValueError(
                "get_filename does not support matrix_combo when file_type is conda_meta"
            )
        return "meta.yaml"
    file_type_prefix = ""
    file_ext = ""
...
def get_output_dir(file_type, config_file_path, file_config, file_name):
    ...
    if file_type == str(OutputTypes.CONDA_META):
        # Note that this directory is not configurable! An alternative would be
        # to ignore the [files] entry and use the dir here to specify the
        # specific recipe, but I think the current approach is a little
        # clearer.
        path.append(
            os.path.join(
                file_config.get("conda_meta_dir", default_conda_meta_dir), file_name
            )
        )
    if file_type == str(OutputTypes.CONDA):
        ...
        for file_type in file_types_to_generate:
            for matrix_combo in grid(file_config.get("matrix", {})):
                ...
                if isinstance(includes, list):
                    assert file_type != str(OutputTypes.CONDA_META)
                    dependencies = []
                    ...
                    assert file_type == str(OutputTypes.CONDA_META)
                    dependencies = {}
                    for section, section_includes in includes.items():
                        dependencies[section] = []
                        for include in section_includes:
                            dependencies[section].extend(
                                get_deps(
                                    parsed_config["dependencies"][include],
                                    file_type,
                                    matrix_combo,
                        ...
                        dependencies[section] = dedupe(dependencies[section])
                else:
                    raise ValueError(
                        f"Unexpected type {type(includes)} for includes list"
                    )
                ...
                if to_stdout:
                    ...
                    if file_type == str(OutputTypes.CONDA_META):
                        if not os.path.exists(file_path):
                            raise ValueError(
                                "meta.yaml file must already exist for the "
                                "rapids_dependency_file_generator to run."
                            )
                        # meta.yaml files are not strictly YAML-compliant. For
                        # example, they may have additional metadata in the
                        # form of e.g. variables declarations {% ... %}
                        # Rather than trying to write valid YAML, we just write
                        # the requirements section as valid YAML and then
                        # overwrite that into the meta.yaml file manually.
                        file_data = StringIO()
                        with file_data as f_tmp:
                            with open(file_path) as f:
                                while line := f.readline():
                                    # Support both first-time files and files
                                    # that already have the header.
                                ...
                                # Now find the first section after the
                                # requirements. Assumes that all top-level keys in
                                # conda's meta.yaml will always be alpha.
                                while line := f.readline():
                                    if re.match("^[a-z]", line):
                                        break
                                f_tmp.write(contents)
                                # Print the start of the next section that was
                                # already discovered.
                                f_tmp.write("\n")
                                f_tmp.write(line)
                                while line := f.readline():
