...
def _get_pyproject_location(extras: typing.Union[_config.FileExtras, None]) -> tuple[str, str]:
    if extras is None:
    ...
    if extras.table == "build-system":
        ...
        if extras.key is not None:
            raise ValueError("The 'key' field is not allowed for the 'pyproject' file type when 'table' is 'project'.")
        key = "dependencies"
    else:
    ...
    return extras.table, key


def _update_pyproject_document(
    *,
    document: tomlkit.TOMLDocument,
    table_name: str,
    key: str,
    dependencies: typing.Sequence[typing.Union[str, dict[str, list[str]]]],
    relative_path_to_config_file: str,
) -> None:
...
def make_dependency_file(
    *,
    ...
    if file_type == _config.Output.CONDA:
        ...
        table_name, key = _get_pyproject_location(extras)
        with open(os.path.join(output_dir, file_name)) as f:
            document = tomlkit.load(f)
        _update_pyproject_document(
            document=document,
            table_name=table_name,
            key=key,
            dependencies=dependencies,
            relative_path_to_config_file=relative_path_to_config_file,
        )
        file_contents = tomlkit.dumps(document)
...
def _build_generation_requests(
    *,
    ...
    if to_stdout and len(file_keys) > 1:
        # `output` is technically a set because of
        # https://github.com/rapidsai/dependency-file-generator/pull/74, but
        # since https://github.com/rapidsai/dependency-file-generator/pull/79
        # it is only ever an exactly-1-item set when stdout=True.
        err_msg = (
...
def _render_generation_requests(
    *,
    requests: list[_GenerationRequest],
    config_file: os.PathLike,
    conda_channels: list[str],
    ...
    rendered_stdout: list[str] = []
    rendered_files: dict[str, str] = {}
    destination_order: list[str] = []
    destination_paths: dict[str, str] = {}
    pyproject_documents: dict[str, tomlkit.TOMLDocument] = {}

    for request in requests:
        if to_stdout:
            rendered_stdout.append(
        ...
        if destination_key not in destination_paths:
            destination_order.append(destination_key)
            destination_paths[destination_key] = file_path

        if request.file_type == _config.Output.PYPROJECT:
            table_name, key = _get_pyproject_location(request.extras)
            if destination_key not in pyproject_documents:
                with open(file_path) as f:
                    pyproject_documents[destination_key] = tomlkit.load(f)
            _update_pyproject_document(
                document=pyproject_documents[destination_key],
                table_name=table_name,
                key=key,
                dependencies=request.dependencies,
                relative_path_to_config_file=os.path.relpath(config_file, request.output_dir),
            )
        else:
            rendered_files[destination_key] = make_dependency_file(
                file_type=request.file_type,
                conda_env_name=request.conda_env_name,
                file_name=request.file_name,
                config_file=config_file,
    ...
    for destination_key, document in pyproject_documents.items():
        rendered_files[destination_key] = tomlkit.dumps(document)

    return _RenderedOutputs(
        stdout=rendered_stdout,
        files={
            destination_paths[destination_key]: rendered_files[destination_key] for destination_key in destination_order
        },
    )


def _publish_rendered_outputs(
    rendered: _RenderedOutputs,
    *,
    clean: typing.Union[str, None],
) -> None:
    if clean is not None:
        delete_existing_files(clean)

    for contents in rendered.stdout:
        print(contents)

    for file_path, contents in rendered.files.items():
...
def make_dependency_files(
    *,
    ...
    to_stdout: bool,
    clean: typing.Union[str, None] = None,
) -> None:
    ...
    to_stdout : bool
        ...
        config_file_path.
    clean : str | None
        A directory tree from which previously generated files should be
        deleted after rendering succeeds and before publishing new outputs.
    ...
    requests = _build_generation_requests(
    ...
    rendered = _render_generation_requests(
        requests=requests,
        config_file=parsed_config.path,
        conda_channels=conda_channels,
        to_stdout=to_stdout,
    )
    _publish_rendered_outputs(rendered, clean=clean)
